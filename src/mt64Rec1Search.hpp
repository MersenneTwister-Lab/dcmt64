#ifndef MT64REC1SEARCH_HPP
#define MT64REC1SEARCH_HPP
/**
 * @file mt64Search.hpp
 *
...
namespace MTToolBox {
     ...
     */
    class mt64rec1 : public TemperingCalculatable<uint64_t> {
    public:
        /**
         * Constructor by mexp.
         * @param mexp Mersenne Exponent
         */
        mt64rec1(int mexp, int id) {
            size = mexp / 64 + 1;
         ...
         */
        mt64rec1(const mt64rec1& src) : param(src.param) {
            size = src.size;
            state = new uint64_t[size];
         ...
         * @param src_param
         */
        mt64rec1(const mt64rec1_param& src_param) :
        ...
        mt64rec1 * clone() const {
            return new mt64rec1(*this);
        }
        ...
        void add(EquidistributionCalculatable<uint64_t>& other) {
            mt64rec1 *that = dynamic_cast<mt64rec1 *>(&other);
            if (that == 0) {
        ...
        bool equals(const mt64rec1& that) {
            for (int i = 0; i < size; i++) {
    ...
    private:
        ...
        mt64rec1& operator=(const mt64rec1&) {
